...
The original brief asked for four things that can't be *guaranteed*. Each was translated into an engineering condition that produces them in expectation:
...
| "Ranked #1 on search engines" | Technical SEO done to spec (sitemap, JSON-LD, Core Web Vitals green, `llms.txt`, strong README). Ranking is *earned*, not promised. |
...
> **Verification note:** the configs above are *written* but not yet *run end-to-end* in this session — `pnpm install` and a CI dry-run are the next steps before declaring Phase 0 verified.
